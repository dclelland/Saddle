...
public struct Matrix2<Scalar> where Scalar: SIMDScalar {
    ...
    public init(shape: Shape, repeating element: SIMDVector) {
        self.init(
    ...
    public init(shape: Shape, elements: [SIMDVector]) {
        self.init(
    ...
    public init(shape: Shape, _ closure: @autoclosure () throws -> SIMDVector) rethrows {
        var elements: [SIMDVector] = []
    ...
    public init(shape: Shape, _ closure: (_ row: Int, _ column: Int) throws -> SIMDVector) rethrows {
        var elements: [SIMDVector] = []
        elements.reserveCapacity(shape.count)
...
extension Matrix2 {
    ...
    public var elements: [SIMDVector] {
        return Array(self)
    }
    
    public var grid: [[SIMDVector]] {
...
extension Matrix2 {
    ...
    public subscript(row: Int, column: Int) -> SIMDVector {
        get {
            precondition(shape.contains(row: row, column: column))
            return SIMDVector(x[row, column], y[row, column])
        }
        set {
            precondition(shape.contains(row: row, column: column))
...
extension Matrix2: ExpressibleByArrayLiteral {
    ...
    public init(arrayLiteral elements: [SIMDVector]...) {
...
extension Matrix2: Collection {
    ...
    public subscript(_ index: Index) -> SIMDVector {
        return SIMDVector(x[index], y[index])
    }
